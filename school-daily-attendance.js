document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadDailyBadges() {
    const schoolId = window.getActiveSchoolId?.();
    if (!schoolId) return;
    const { data, error } = await db.from('siap_school_daily_attendance_current').select('student_id,status,percentage').eq('school_id',schoolId);
    if (error) return;
    dailyBadges.clear();
    (data || []).forEach(item => dailyBadges.set(item.student_id, STATUS[item.status] ? item.status : classify(Number(item.percentage))));
    window.render?.();
  }
  ...
  window.getSiapAttendanceBadge = studentId => {
    ...
    const statuses = [teacherStatus?.(studentId), dailyBadges.get(studentId)].filter(Boolean);
    const key = statuses.sort((left,right)=>weight[right]-weight[left])[0];
  ...
  by('[data-sda-read]').onclick = async () => {
    ...
    reading = true;const control=by('[data-sda-read]');control.disabled=true;render('Lendo as datas da turma no SIAP. Isso pode levar alguns minutos…');
    try {
      ...
      reading=false;control.disabled=false;
    }
  };
