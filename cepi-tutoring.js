document.addEventListener('DOMContentLoaded', () => {
  ...
  tutorModal.innerHTML = `<section class="modal small"><div class="modal-head"><div><h3>Cadastrar tutor</h3><div class="meta">O tutor pode ter ou não acesso ao CARÔMETRO.</div></div><button class="close" type="button" data-cepi-close="cepiTutorModal">×</button></div><form id="cepiTutorForm" class="form">
  ...
  [modal, tutorModal, assignmentModal, reportModal].forEach(item => item.onclick = event => { if (event.target === item) closeModal(item.id); });
  ...
  function renderTutoring() {
    ...
    target.innerHTML = tutors.map(tutor => {
      const tutorAssignments = assignments.filter(item => item.tutor_id === tutor.id);
      ...
      const rows = tutorAssignments.length ? tutorAssignments.map(item => {
        ...
        return `<div class="cepi-student-row"><div class="cepi-student-photo">${photo}</div><div class="cepi-student-info"><b>${escapeHtml(student.name || 'Aluno')}</b><small>${escapeHtml(student.className || 'Turma não informada')}</small><div class="cepi-student-labels"><span>Tutor(a): ${escapeHtml(tutor.display_name)}</span><span>Conselheiro(a): ${escapeHtml(counselorNames.join(' · ') || 'não definido')}</span>${student.report ? `<span>Observações: ${escapeHtml(student.report)}</span>` : ''}</div><div class="cepi-student-status">${tutorStatusHtml(student, activity)}</div></div><div class="cepi-row-actions"><button class="btn secondary" type="button" data-tutoring-details="${escapeHtml(item.student_id)}">Ver detalhes</button><span class="cepi-form-pending">Ficha em preparação</span>${access.can_manage ? `<button class="btn secondary" type="button" data-end-assignment="${escapeHtml(item.id)}">Encerrar vínculo</button>` : ''}</div><div class="cepi-student-expanded hidden" data-tutoring-panel="${escapeHtml(item.student_id)}"><section><b>Ocorrências</b><ul>${occurrences}</ul></section><section><b>Frequência</b><ul>${attendance}</ul></section><section><b>Livro/Revisa</b><ul>${books}</ul></section></div></div>`;
      }).join('') : '<div class="cepi-empty">Nenhum tutorando atribuído.</div>';
      return `<section class="cepi-tutor-group"><div class="cepi-tutor-head"><div><b>${escapeHtml(tutor.display_name)}</b><small>${escapeHtml(type)}${tutor.email ? ` · ${escapeHtml(tutor.email)}` : ''}</small></div><span class="pill light">${tutorAssignments.length} tutorando(s)</span></div><div class="cepi-students">${rows}</div></section>`;
    }).join('');
  ...
  document.getElementById('cepiTutorForm').onsubmit = async event => {
    ...
    const { error } = await db.from('cepi_tutors').insert(row);
    if (error) { toast(error.code === '23505' ? 'Este usuário já está cadastrado como tutor.' : error.message); return; }
    closeModal('cepiTutorModal'); toast('Tutor cadastrado.'); await loadTutoring();
  };
  ...
  function openAssignmentForm() {
    ...
    document.getElementById('cepiAssignmentStudents').innerHTML = students.filter(item => !alreadyAssigned.has(item.id)).sort((a,b) => a.name.localeCompare(b.name, 'pt-BR')).map(item => `<option value="${escapeHtml(item.id)}">${escapeHtml(item.name)} — ${escapeHtml(item.className || 'Sem turma')}</option>`).join('');
  ...
  function openReportForm() {
    const select = document.getElementById('cepiReportStudent');
    select.innerHTML = '<option value="">Selecione</option>' + assignments.map(item => {
