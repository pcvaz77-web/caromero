document.addEventListener('DOMContentLoaded', () => {
  ...
  new MutationObserver(() => { if (detailsPanel.classList.contains('hidden')) resetDetailsPosition(); }).observe(detailsPanel, { attributes: true, attributeFilter: ['class'] });
  let dragOffsetX = 0;
  let dragOffsetY = 0;
  let draggingDetails = false;
  detailsPanel.addEventListener('pointerdown', event => {
    if (window.matchMedia('(max-width: 800px)').matches || !event.target.closest('.detail-head')) return;
    ...
    detailsPanel.setPointerCapture(event.pointerId);
    event.preventDefault();
  });
  detailsPanel.addEventListener('pointermove', event => {
  ...
  const stopDraggingDetails = () => { draggingDetails = false; detailsPanel.classList.remove('is-dragging'); };
  detailsPanel.addEventListener('pointerup', stopDraggingDetails);
  detailsPanel.addEventListener('pointercancel', stopDraggingDetails);
