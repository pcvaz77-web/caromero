...
test('nao carrega o modulo de importacao de frequencia', () => {
  ...
  assert.match(index, /permissions-and-details\.js\?v=\d+/);
...
test('concessoes permitem busca e preservam a escola aberta ao atualizar', () => {
  ...
  assert.match(dashboard, /Renovar concessão/);
  assert.match(dashboard, /Liberação até/);
  assert.match(dashboard, /platform-siap-product-status/);
