document.addEventListener('DOMContentLoaded', () => {
  ...
  const isAdvancedUser = () => permission.role === 'admin' || !!permission.is_coordinator || !!permission.is_secretary;
  ...
  const canEdit = () => permission.role === 'admin' || (isGeneralTeacher() && (!!permission.can_edit_students || !!permission.can_edit_guardian_contact)) || (isAdvancedUser() && (!!permission.can_edit_all || !!permission.can_edit_photo || !!permission.can_edit_name || !!permission.can_edit_class || !!permission.can_edit_report || !!permission.can_edit_guardian_contact));
  ...
  const permissionFields = ['can_add_students', 'can_edit_students', 'can_delete_students', 'can_edit_all', 'can_edit_photo', 'can_edit_name', 'can_edit_class', 'can_edit_report', 'can_edit_guardian_contact', 'can_manage_observation_options', 'can_invite_teachers', 'can_manage_member_permissions', 'can_view_uniform', 'can_edit_uniform', 'can_mark_all_uniform_received', 'can_view_occurrences', 'can_register_occurrences', 'can_edit_occurrences', 'can_delete_occurrences', 'can_manage_counselors', 'can_view_class_summary'];
  const secretaryPermissionFields = permissionFields.filter(field => field !== 'can_manage_counselors').concat('can_import_school_daily_attendance', 'can_receive_notifications');
  const sessionPermissionFields = [...permissionFields, 'can_import_siap_attendance', 'can_import_school_daily_attendance', 'can_receive_notifications'];
  ...
  const schoolPermissionSelect = 'can_add_students,can_edit_students,can_delete_students,can_edit_all,can_edit_photo,can_edit_name,can_edit_class,can_edit_report,can_edit_guardian_contact,can_manage_observation_options,can_invite_teachers,can_manage_member_permissions,can_view_uniform,can_edit_uniform,can_mark_all_uniform_received,can_view_occurrences,can_register_occurrences,can_edit_occurrences,can_delete_occurrences,can_manage_counselors,can_view_class_summary,can_use_siap_assistant,can_import_siap_attendance,can_import_school_daily_attendance,can_receive_notifications,can_prepare_school_year';
  ...
  async function loadSchoolPermissions(schoolId) {
    ...
    const { data:panelRows, error:panelError } = await db.from('school_members')
      .select('id,school_member_permissions(can_view_class_summary,can_edit_guardian_contact,can_use_siap_assistant,can_import_siap_attendance,can_import_school_daily_attendance,can_receive_notifications,can_prepare_school_year)')
    ...
    const panelPermissionByMember = new Map((panelRows || []).map(item => {
      ...
      return [item.id, {
        ...
        can_edit_guardian_contact:!!rights?.can_edit_guardian_contact,
        ...
        can_receive_notifications:!!rights?.can_receive_notifications,
  ...
  const occCheck = (item, occMap, key, label) => {
    const action = key === 'can_edit_occurrences' || key === 'can_delete_occurrences';
    if (item.is_secretary && action) return '';
    const admin = item.role === 'admin';
    const target = occMap.get(item.user_id);
    const disabled = admin || !target || target.member_status !== 'active' || (!action && !!target.can_edit_all);
    const checked = !!target && (target[key] || (!action && target.can_edit_all));
  ...
  window.setOccurrencePermission = async (memberId, key, value) => {
    ...
    if (error) { toast(error.message); await openPermissions(); return; }
  ...
  const coordinatorPermissionOptions = [
    ...
    ['can_edit_occurrences', 'Editar ocorrências da escola'],
    ['can_delete_occurrences', 'Excluir ocorrências da escola'],
  ...
  function delegatedOccurrenceChecks(item, actorRights) {
    return [
      ['can_edit_occurrences', 'Editar ocorrências que registrou'],
      ['can_delete_occurrences', 'Excluir ocorrências que registrou']
    ].map(([key, label]) => {
      const checked = !!item[key];
      const disabled = item.member_status !== 'active' || item.user_id === user?.id || (!checked && actorRights[key] !== true);
      return `<label class="check"><input type="checkbox" ${checked ? 'checked' : ''} ${disabled ? 'disabled' : ''} onchange="setOccurrencePermission('${item.member_id}','${key}',this.checked)"> ${label}</label>`;
    }).join('');
  }
  ...
  async function openPermissions() {
    ...
    const schoolDailyAttendanceCheck = item => check(item,'can_import_school_daily_attendance','Usar Frequência da Secretaria',false);
    const notificationCheck = item => check(item,'can_receive_notifications','Receber notificações',false);
    ...
    const cards = sortedUsers.map(item => {
      ...
      if (item.is_secretary) return `<article class="perm ${active ? '' : 'member-suspended'}" data-permission-scope="advanced" data-search="${esc(`${name} ${email}`.toLowerCase())}"><div class="permission-user"><b>${esc(name)}</b><div class="meta">${esc(email)} · Secretaria</div></div><div class="permission-primary">${check(item,'can_edit_all','Liberar todas as permissões disponíveis',false)}</div>${access}<div class="permission-basic">${check(item,'can_add_students','Pode adicionar',false)}${check(item,'can_delete_students','Pode excluir',false)}${check(item,'can_view_class_summary','Visualizar Painel da Turma',false)}${schoolDailyAttendanceCheck(item)}${notificationCheck(item)}</div><details class="coordinator-right-group"><summary>Cadastro, gestão, uniforme e ocorrências</summary><div class="edit-rights">${check(item,'can_edit_photo','Editar somente foto',false)}${check(item,'can_edit_name','Editar somente nome',false)}${check(item,'can_edit_guardian_contact','Editar dados do responsável',false)}${check(item,'can_edit_class','Editar somente mudança de turma',false)}${check(item,'can_edit_report','Pode editar observações do aluno',false)}${check(item,'can_manage_observation_options','Gerenciar opções de observação',false)}${check(item,'can_invite_teachers','Convidar professores',false)}${check(item,'can_manage_member_permissions','Gerenciar professores',false)}${check(item,'can_view_uniform','Visualizar Uniforme',false)}${check(item,'can_edit_uniform','Editar Uniforme e material',false)}${check(item,'can_mark_all_uniform_received','Marcar todos como receberam',false)}${occCheck(item,occMap,'can_view_occurrences','Visualizar Ocorrências')}${occCheck(item,occMap,'can_register_occurrences','Registrar Ocorrência')}${occCheck(item,occMap,'can_edit_occurrences','Editar ocorrências da escola')}${occCheck(item,occMap,'can_delete_occurrences','Excluir ocorrências da escola')}</div></details><div class="meta">Secretaria nunca pode ser indicada como conselheiro de turma.</div></article>`;
      if (!isCoordinator(item)) return `<article class="perm ${active ? '' : 'member-suspended'}" data-permission-scope="general" data-search="${esc(`${name} ${email}`.toLowerCase())}"><div class="permission-user"><b>${esc(name)}</b><div class="meta">${esc(email)} · Acesso de professor(a)</div></div><div class="permission-basic">${check(item,'can_add_students','Pode adicionar',false).replace('setUserPermission','setGeneralPermission')}${check(item,'can_edit_students','Pode editar e excluir',false).replace('setUserPermission','setGeneralPermission')}${check(item,'can_edit_guardian_contact','Editar dados do responsável',false).replace('setUserPermission','setGeneralPermission')}${check(item,'can_view_class_summary','Visualizar Painel da Turma',false).replace('setUserPermission','setGeneralPermission')}${occCheck(item,occMap,'can_edit_occurrences','Editar ocorrências que registrou')}${occCheck(item,occMap,'can_delete_occurrences','Excluir ocorrências que registrou')}</div><div class="meta">A Frequência Assistida fica disponível automaticamente no Painel da Turma quando este professor é o conselheiro responsável.</div>${access}</article>`;
      return `<article class="perm ${active ? '' : 'member-suspended'}" data-permission-scope="advanced" data-search="${esc(`${name} ${email}`.toLowerCase())}"><div class="permission-user"><b>${esc(name)}</b><div class="meta">${esc(email)}${admin ? ' · Administrador principal' : ' · Coordenador'}</div></div><div class="permission-primary">${check(item,'can_edit_all','Editar tudo',admin)}</div>${access}<div class="permission-basic">${check(item,'can_add_students','Pode adicionar',admin)}${check(item,'can_delete_students','Pode excluir',admin)}${check(item,'can_view_class_summary','Visualizar Painel da Turma',admin)}${yearCheck(item,admin)}</div><details class="coordinator-right-group"><summary>Cadastro, gestão, uniforme e ocorrências</summary><div class="edit-rights">${check(item,'can_edit_photo','Editar somente foto',admin)}${check(item,'can_edit_name','Editar somente nome',admin)}${check(item,'can_edit_guardian_contact','Editar dados do responsável',admin)}${check(item,'can_edit_class','Editar somente mudança de turma',admin)}${check(item,'can_edit_report','Pode editar observações do aluno',admin)}${check(item,'can_manage_observation_options','Gerenciar opções de observação',admin)}${check(item,'can_invite_teachers','Convidar professores',admin)}${check(item,'can_manage_member_permissions','Gerenciar permissões de professores',admin)}${check(item,'can_view_uniform','Visualizar Uniforme',admin)}${check(item,'can_edit_uniform','Editar Uniforme e material',admin)}${check(item,'can_mark_all_uniform_received','Marcar todos como receberam',admin)}${check(item,'can_manage_counselors','Gerenciar conselheiros de turma',admin)}${occCheck(item,occMap,'can_view_occurrences','Visualizar Ocorrências')}${occCheck(item,occMap,'can_register_occurrences','Registrar Ocorrência')}${occCheck(item,occMap,'can_edit_occurrences','Editar ocorrências da escola')}${occCheck(item,occMap,'can_delete_occurrences','Excluir ocorrências da escola')}</div></details></article>`;
    }).join('');
  ...
  window.setUserPermission = async (id, key, value) => {
    const membership = await currentSchoolMembershipOrWarn();
    if (!membership) return;
    const { data:targetMember } = await db.from('school_members').select('id,role').eq('user_id', id).eq('school_id', membership.school_id).eq('status', 'active').maybeSingle();
    const memberId = targetMember?.id || null;
    if (!memberId) { toast('Este usuário não pertence à escola ativa.'); return; }
    const commercialUpdates = key === 'can_edit_all'
      ? (targetMember.role === 'secretary' ? secretaryPermissionFields : permissionFields).map(permissionKey => [permissionKey, value])
      : [[key, value]];
    const { error } = await db.rpc('set_school_member_permissions_batch', {
      target_member_id:memberId,
  ...
  window.setSchoolYearPermission = async (id, value) => {
    ...
    const memberId = await findSchoolMemberId(id, membership.school_id);
    if (!memberId) { toast('Este usuário não pertence à escola ativa.'); return; }
    const { error } = await db.rpc('set_school_year_transition_permission', { target_member_id:memberId, permission_value:value });
    if (error) { toast(error.message); return; }
    toast('Permissão do ano letivo atualizada.');
    openPermissions();
  };
